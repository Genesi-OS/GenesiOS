...
_WORKFLOW_KINDS = {
    ...
    "evt_manual":      [],
    "evt_schedule":    ["interval", "time", "cron", "varName"],
    "evt_fs":          ["path", "pattern", "change", "recursive", "varName"],
    "evt_resource":    ["metric", "op", "threshold", "varName"],
    "evt_temperature": ["threshold", "sensor", "varName"],
    ...
    "evt_process":     ["app", "metric", "threshold", "varName"],
    "evt_power":       ["event", "level", "varName"],
    "evt_disk":        ["event", "path", "threshold", "varName"],
    "evt_usb":         ["action", "varName"],
    "evt_network":     ["event", "iface", "varName"],
    "evt_bluetooth":   ["action", "name", "varName"],
    "evt_idle":        ["event", "minutes", "varName"],
    "evt_hotkey":      ["combo", "varName"],
    "evt_startup":     [],
    "evt_log":         ["path", "pattern", "varName"],
    # match, not pattern: "pattern" was dropped by the validator, so a
    # generated command sensor set to "output matches" matched against an
    # empty regex. interval was missing outright, so it always polled at
    # the default.
    "evt_command":     ["command", "on", "match", "interval", "varName"],
    "evt_clipboard":   ["contains", "onlyUrl", "varName"],
    "evt_screenshot":  ["path", "varName"],
    "evt_webhook":     ["path", "port", "token", "bindAll", "varName"],
    # action kind
    "act_script":      ["command", "terminal", "varName"],
    "act_ai":          ["prompt", "model", "exec", "outputs", "turbo", "aiMode", "varName"],
    "act_cond":        ["mode", "expr", "prompt", "model", "varName"],
    "act_loop":        ["source", "list", "from", "to", "max", "varName"],
    "act_subflow":     ["workflow"],
    "act_notify":      ["title", "body"],
    "act_email":       ["mode", "account", "host", "port", "user", "to",
                        "subject", "body", "folder", "limit", "varName"],
    "act_http":        ["url", "method", "body", "varName"],
    ...
    "act_file":        ["op", "src", "dest", "varName"],
    "act_app":         ["app", "op", "varName"],
    "act_sound":       ["sound"],
    "act_wait":        ["seconds"],
    "act_power":       ["op"],
}
